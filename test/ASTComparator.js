...
const Class = require('../src/AST/Class');
const ComparisonFailure = Jymfony.Component.Testing.Comparator.ComparisonFailure;
const NodeInterface = require('../src/AST/NodeInterface');
const Function = require('../src/AST/Function');

/**
 * Compares objects for equality.
 *
 * @memberOf Jymfony.Compiler.Tests
 */
export default class ASTComparator extends ArrayComparator {
    /**
     * @inheritdoc
     */
    accepts(expected, actual) {
        return expected instanceof NodeInterface && actual instanceof NodeInterface;
    }
    ...
    toObjectLiteral(value) {
        const obj = super.toObjectLiteral(value);
        ...
        if (value instanceof Function && value.id !== null && (value.id.name || '').startsWith('_anonymous_xΞ')) {
            obj._id = null;
        }
